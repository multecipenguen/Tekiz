...
@Component
export default class RoleListComponent extends TekizComponentBase {
  ...
  editRole(id: string) {
    this.formTitle = id
      ? this.$t("EditRole").toString()
      : this.$t("NewRole").toString();
    this.errors = [];
    this.TekizService.get<IGetRoleForCreateOrUpdateOutput>(
      "/api/roles/" + (id ? id : Guid.empty)
    ).then((response) => {
  ...
  deleteRole(id: string) {
    this.swalConfirm(this.$t("AreYouSureToDelete").toString()).then(
      (result) => {
        if (result.value) {
          this.TekizService.delete("/api/roles?id=" + id).then((response) => {
            if (!response.isError) {
  ...
  save() {
    if (this.refs.form.validate()) {
      this.errors = [];
      this.TekizService.postOrPut<void>(
        "/api/roles",
  ...
  selectAllPermissions() {
    ...
    if (this.selectAll) {
      this.createOrUpdateRoleInput.grantedPermissionIds = this.allPermissions.map(
