<template>
  <div>
    <v-toolbar flat color="white">
      ...
      <v-spacer></v-spacer>
      <v-btn
        v-if="Tekiz.auth.isGranted('Permissions_Role_Create')"
        @click="editRole()"
        color="primary"
        dark
        class="mb-2"
        ...
        <v-card>
          ...
          <v-card-text>
            ...
            <v-form ref="form" @submit.prevent="save">
              <v-text-field
                v-model="createOrUpdateRoleInput.role.name"
                :label="$t('RoleName')"
                :rules="[requiredError]"
              ></v-text-field>
              <v-list dense>
      ...
      <template v-slot:item.action="{ item }">
        <v-icon
          v-if="
            ...
            Tekiz.auth.isGranted('Permissions_Role_Update')
          "
          small
          class="mr-2"
        ...
        <v-icon
          v-if="
            !item.isSystemDefault &&
            Tekiz.auth.isGranted('Permissions_Role_Delete')
          "
          small
