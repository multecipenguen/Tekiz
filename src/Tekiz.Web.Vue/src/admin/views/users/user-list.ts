...
@Component
export default class UserListComponent extends TekizComponentBase {
  ...
  editUser(id: string) {
    this.formTitle = id
      ? this.$t("EditUser").toString()
      : this.$t("NewUser").toString();
    this.isEdit = id ? true : false;
    this.errors = [];
    this.TekizService.get<IGetUserForCreateOrUpdateOutput>(
      "/api/users/" + (id ? id : Guid.empty)
    ).then((response) => {
  ...
  deleteUser(id: string) {
    this.swalConfirm(this.$t("AreYouSureToDelete").toString()).then(
      (result) => {
        if (result.value) {
          this.TekizService.delete("/api/users?id=" + id).then((response) => {
            debugger;
            if (!response.isError) {
  ...
  getUsers() {
    ...
    this.TekizService.get<IPagedList<IPagedListInput>>(
      "/api/users" + query,
      false
    ).then((response) => {
  ...
  selectAllRoles() {
    ...
    if (this.selectAll) {
      this.createOrUpdateUserInput.grantedRoleIds = this.allRoles.map(
        (roles) => roles.id
      ) as string[];
    }
  }
