<template>
  <div>
    <v-toolbar flat color="white">
      ...
      <v-spacer></v-spacer>
      <v-btn
        v-if="Tekiz.auth.isGranted('Permissions_User_Create')"
        @click="editUser()"
        color="primary"
        dark
        class="mb-2"
          ...
          <v-card-text>
            ...
            <v-form ref="form" @keyup.native.enter="save">
              ...
              <v-text-field
                ...
                :error-messages="
                  passwordMatchError(
                    createOrUpdateUserInput.user.password,
                    createOrUpdateUserInput.user.passwordRepeat
                  )
                "
                :rules="[requiredError]"
              ></v-text-field>
              ...
              <v-list dense>
                <v-list-item v-for="item in allRoles" :key="item.id">
                  <v-checkbox
                    v-model="createOrUpdateUserInput.grantedRoleIds"
                    :label="item.name"
                    :value="item.id"
                  ></v-checkbox>
                </v-list-item>
    ...
    >
      <template v-slot:item.action="{ item }">
        <v-icon
          v-if="Tekiz.auth.isGranted('Permissions_User_Update')"
          small
          class="mr-2"
        ...
        <v-icon
          v-if="Tekiz.auth.isGranted('Permissions_User_Delete')"
          small
